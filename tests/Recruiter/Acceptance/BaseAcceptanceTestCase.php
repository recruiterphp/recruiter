...
abstract class BaseAcceptanceTestCase extends TestCase
{
    ...
    protected function waitForNumberOfWorkersToBe(int $expectedNumber, int $howManySeconds = 15): void
    {
    ...
    protected function startRecruiter(): array
    {
        ...
        Timeout::inSeconds(15, 'recruiter to be up')
    ...
    protected function startCleaner(): array
    {
        ...
        Timeout::inSeconds(15, 'cleaner to be up')
            ->until(function () use ($process) {
                assert(is_resource($process));
                $status = proc_get_status($process);
    ...
    protected function startWorker(array $additionalOptions = []): array
    {
        ...
        Timeout::inSeconds(15, 'worker to be up')
            ->until(function () use ($process) {
                assert(is_resource($process));
                $status = proc_get_status($process);
